...
jsPsych.plugins["picture-match"] = (function () {
  ...
  plugin.trial = function (display_element, trial) {
    ...
    if (trial.info.turn == "partner") {
      // setup stimulus
      var sound = new Audio("audio/" + trial.info.correct[0] + ".mp3");
      ...
      jsPsych.pluginAPI.setTimeout(function () {
        ...
        $("." + trial.info.correct[1]).addClass("highlight");
      }, trial.delay);
    ...
    function end_trial() {
      ...
      if (trial.info.turn == "partner") {
        trial_data.correct =
          trial.info.correct[1] == response.key.replace("arrow", "");
      }
    ...
    function startRecording() {
      ...
      function upload(blob) {
        ...
        fd.append(
          "name",
          `${trial.subject}_${jsPsych.currentTimelineNodeID()}_${
            trial.info.correct[0]
          }_`
        );
    ...
    function after_response(info) {
      ...
      response.key = jsPsych.pluginAPI.convertKeyCodeToKeyCharacter(
        response.key
      );
