...
var debrief_text = `<div class='instructions'>
	...
	<p>If you had to do the number categorisation task when naming the objects, you were in the Numbers Task condition (Condition B). If you didn’t have to categorise numbers, you were in the control condition (Condition A). You were in condition ${condition_assignment.slice(
    -1
  )}.</p>
...
var list_assignment = jsPsych.randomization.sampleWithoutReplacement(
  ["listA", "listB"],
  1
)[0];
...
var practice_comps = jsPsych.randomization.sampleWithoutReplacement(
  filler_comps,
  5
);
var practice_targets = jsPsych.randomization.sampleWithoutReplacement(
  filler_targets,
  5
);
...
window.onbeforeunload = function (e) {
  ...
  if (prevent_leave) {
    ...
    e.preventDefault();
    e.returnValue =
      "Your progress will not be saved if you leave the site. Are you sure you want to continue?";
  } else {
  ...
  loop_function: (data) => {
    if (
      data
        ...
        .count() <
      practice_variables.length - 1
    ) {
      alert(
        `You got less than ${
          practice_variables.length - 1
        } correct answers, please try again.`
      );
      return true;
    }
    return false;
  ...
  timeline: [
    {
      ...
      key_forward: "enter",
      pages: [
        ` <p>Between naming and matching turns you will also complete a memory task.</p>
...
var check_stroop = {
  ...
  timeline: [
    {
      stimulus: function () {
        var choice = jsPsych.randomization.sampleWithoutReplacement(
          ["VALUE", "SIZE"],
          1
        )[0];
...
var stroop_practice = {
  timeline: [number_stroop, check_stroop],
  repetitions: 5,
  loop_function: (data) => {
...
var reject_part = {
  ...
  conditional_function: function () {
    ...
    if (
      data.impairment_condition == "Yes" ||
      data.normal_hearing == "No" ||
      data.normal_vision == "No" ||
      (data.native_speaker == "No" && parseInt(data.nonnative_speaker[0]) < 4)
    ) {
      return true;
    } else {
      return false;
...
if (localStorage.getItem("completion_code_PMT")) {
  ...
  alert(
    "You have already completed this experiment. Your completion code is: " +
      completion_code
  );
}
...
//startExperiment(); //Comment out to include microphone check
